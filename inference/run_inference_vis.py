...
def apply_gaussian_filter(sst, land_mask, sigma=1.0, missing_mask=None):
    """高斯滤波 (只对缺失区域，观测值保持不变)

    Args:
        sst: (H, W) composed SST (观测区域=原值, 缺失区域=模型预测)
        land_mask: (H, W) 1=陆地, 0=海洋
        sigma: 高斯核标准差
        missing_mask: (H, W) 1=缺失(模型预测), 0=观测. 若None则对整个海洋区域滤波
    """
    result = sst.copy()
...
def main():
    ...
    for i, day_idx in enumerate(day_indices):
        ...
        fno_gaussian = apply_gaussian_filter(fno_composed, land_mask, sigma=GAUSSIAN_SIGMA, missing_mask=artificial_mask)
