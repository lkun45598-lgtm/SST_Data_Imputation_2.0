...
class JAXAFinetuneDataset(Dataset):
    ...
    def __init__(self, data_dir, series_ids=None, window_size=30,
        ...
        Args:
            ...
            seed: 随机种子
            stride: 未使用，保留兼容
            hour_offset: 固定时刻（0-23），只取每天该时刻的帧
        """
        ...
        for sid in series_ids:
            # 尝试两种文件名格式
            h5_path = self.data_dir / f'jaxa_filtered_{sid:02d}.h5'
            if not h5_path.exists():
                h5_path = self.data_dir / f'jaxa_knn_filled_{sid:02d}.h5'
            if not h5_path.exists():
                print(f"Warning: Series {sid:02d} not found, skipping...")
                continue
        ...
        # 预计算每个序列中固定时刻的日级帧索引
        # hour_offset=0 表示取每天00:00, =1表示取每天01:00, ...
        self.daily_indices = []  # [(series_idx, local_frame_idx), ...]
        for si, info in enumerate(self.series_info):
            num_frames = info['num_frames']
            # 从hour_offset开始，每24帧取一帧
            for frame_idx in range(self.hour_offset, num_frames, 24):
                self.daily_indices.append((si, frame_idx))

        self.total_days = len(self.daily_indices)

        print(f"\nJAXA Finetune Dataset initialized:")
        print(f"  Data dir: {self.data_dir}")
        print(f"  Series: {[s['series_id'] for s in self.series_info]}")
        print(f"  Total frames: {self.total_frames}")
        print(f"  Total days (hour={hour_offset:02d}:00): {self.total_days}")
        print(f"  Window size: {window_size}")
        print(f"  Samples: {max(0, self.total_days - self.window_size + 1)}")
    ...
    def __len__(self):
        # 样本数 = 总天数 - 窗口大小 + 1
        return max(0, self.total_days - self.window_size + 1)
    ...
    def _load_frame(self, series_idx, local_idx):
        ...
        with h5py.File(info['h5_path'], 'r') as f:
            ...
            # 兼容新旧数据格式
            if 'original_missing_mask' in f:
                original_missing_mask = f['original_missing_mask'][local_idx].astype(np.float32)
            else:
                # 新数据：从obs_mask计算missing_mask
                original_missing_mask = 1.0 - original_obs_mask
    ...
    def __getitem__(self, idx):
        # idx对应第idx个样本，目标天是 idx + window_size - 1
        target_day_idx = idx + self.window_size - 1
        target_series_idx, target_local_idx = self.daily_indices[target_day_idx]

        # 连续读取30天同一时刻的帧
        sst_seq = []
        mask_seq = []

        for day in range(self.window_size):
            day_idx = idx + day
            si, fi = self.daily_indices[day_idx]
            # 确保在同一个序列内
            if si != target_series_idx:
                # 跨序列时用目标序列的最早帧填充
                si = target_series_idx
                fi = self.daily_indices[idx + self.window_size - 1][1]
            frame_data = self._load_frame(si, fi)
        ...
        # 第30天的数据（目标帧）
        target_data = self._load_frame(target_series_idx, target_local_idx)
