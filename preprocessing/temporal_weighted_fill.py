...
Algorithm:
    ...
    observations within lookback window, with diurnal cycle awareness:

    hour_diff = min(|Δh| % 24, 24 - |Δh| % 24)   # circular hour distance
    weight(t_history) = (1 / dt) * exp(-α * hour_diff)
    filled_value = sum(w_i * v_i) / sum(w_i)

    This ensures same-hour-of-day observations dominate, reducing temporal
    aliasing from diurnal SST variation (~2.3K).
...
def fill_frame_vectorized(target_sst: np.ndarray,
    ...
    """Vectorized temporal weighted filling with diurnal cycle awareness.

    For each missing pixel, computes weighted average of historical observations.
    Weight combines inverse time distance with diurnal cycle penalty:

        hour_diff = min(|Δh| % 24, 24 - |Δh| % 24)  # circular hour distance
        w = (1 / dt) * exp(-α * hour_diff)

    This ensures same-hour-of-day observations dominate the fill, reducing
    temporal aliasing from diurnal SST variation (~2.3K).
    """
    ...
    target_hour_of_day = target_hour_offset % 24
    ...
    for t, t_sst in history_cache.items():
        ...
        # Circular hour-of-day distance (0~12)
        hist_hour_of_day = t % 24
        hour_diff = abs(target_hour_of_day - hist_hour_of_day)
        hour_diff = min(hour_diff, 24 - hour_diff)

        # Combined weight: inverse time distance * diurnal decay
        weight = (1.0 / time_distance) * np.exp(-DIURNAL_ALPHA * hour_diff)
