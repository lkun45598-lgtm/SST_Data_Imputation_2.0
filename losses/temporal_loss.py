...
def boundary_smoothness_loss(pred, target, missing_mask, ocean_mask):
    """
    边界平滑损失：约束缺失区域和观测区域交界处的梯度连续性

    通过膨胀操作找到边界区域，在边界处计算梯度匹配
    ...
    """
    # 找到缺失区域的边界（膨胀1像素）
    kernel = torch.ones(1, 1, 3, 3, device=missing_mask.device)
    missing_dilated = F.conv2d(missing_mask.unsqueeze(1).float(), kernel, padding=1)
    missing_dilated = (missing_dilated > 0).float().squeeze(1)

    # 边界 = 膨胀后的区域 - 原始缺失区域
    boundary_mask = (missing_dilated - missing_mask) * ocean_mask
    ...
    # 边界mask对应的梯度区域
    boundary_y = boundary_mask[:, :, 1:, :]
    boundary_x = boundary_mask[:, :, :, 1:]

    # 计算边界处的梯度差异
    grad_diff_y = torch.abs(pred_grad_y - target_grad_y) * boundary_y
    grad_diff_x = torch.abs(pred_grad_x - target_grad_x) * boundary_x

    loss = (grad_diff_y.sum() / (boundary_y.sum() + 1e-8) +
            grad_diff_x.sum() / (boundary_x.sum() + 1e-8)) / 2
...
def combined_loss_temporal(pred, target, missing_mask, ocean_mask,
                           ...
                           beta_laplacian=0.15, beta_boundary=0.2):
    """
    ...
    Args:
        ...
        beta_boundary: 边界平滑约束权重 (默认0.2，约束缺失/观测交界处)
    ...
    # 边界平滑约束（缺失/观测交界处）
    if beta_boundary > 0:
        loss_boundary = boundary_smoothness_loss(pred, target, missing_mask, ocean_mask)
    else:
